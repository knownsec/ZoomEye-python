## ZoomEye-python
`ZoomEye` is a network space search engine, users can search for network devices using a browser <https://www.zoomeye.org>.
...
>since the fields returned by `web-search` and `host-search` interfaces are different, you need to fill in the correct fields when filtering.
>the fields included in `web-search`: app / headers / keywords / title / ip / site / city / country
>the fields included in `host-search`: app / version / device / ip / port / hostname / city / country / asn / banner
...
`ZoomEye-python` is a part of 404Team [Starlink Project](https://github.com/knownsec/404StarLink-Project). If you have any questions about `ZoomEye-python` or want to talk to a small partner, you can refer to The way to join the group of Starlink Project.
